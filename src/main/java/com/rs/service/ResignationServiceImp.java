...
@Service
public class ResignationServiceImp implements ResignationService{
	
	@Autowired
	private EmployeeRepository repo;
	@Autowired
	private ResignationRepository res;
	@Autowired
			...
			DateTimeFormatter format = DateTimeFormatter.ofPattern("dd-MM-yyyy");
			 LocalDate dateOfApplying = LocalDate.parse(data.getDateOfApplying(), format);
		        ...
		        if (!existingResignation.isEmpty()) {
		            throw new DuplicateFound("Leave already applied for the given end date: " + data.getDateOfApplying());
		        }
